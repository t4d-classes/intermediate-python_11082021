...
def get_data_sync(task_num: int) -> None:
    """ get data """
    print(
        f"thread id: {threading.get_ident()}, task: {task_num}, starting get data")

    with open("data\\data.txt", encoding="UTF-8") as f:
        print(
            f"thread id: {threading.get_ident()}, task: {task_num}",
            f.readlines())

    print(
        f"thread id: {threading.get_ident()}, task: {task_num}, finished get data")
...
async def get_data_async(task_num: int) -> None:
    ...
    # async with aiofiles.open("data\\data.txt", encoding="UTF-8") as f:
    # print(f"thread id: {threading.get_ident()}, task: {task_num}", await
    # f.readlines())
